...
function suggestions() {
  ...
  $.getJSON(apiUrl + '/suggest/' + term, function (data) {
    ...
    finalResults.forEach(function (result, i) {
      ...
      var e = $('<li class="' + c + '" align="center">' + result.display + '</li>');
      results.append(e);
      e.click(function () {
        songLyrics(result);
      });
...
function songLyrics(song) {
  ...
  $.getJSON(apiUrl + '/v1/' + song.artist + '/' + song.title, function (data) {
    var html = '<h3 class="lyrics-title">' + song.display + '</h3></br>';
    html += '<div class="copy-lyrics" id="copy-lyrics" data-clipboard-target="#thelyrics"><button id="copy-ok" class="btn btn-info">Copy Lyrics</button></div>';
    html += '</br><div id="thelyrics">' + data.lyrics.replace(/\n/g, '<br />') + '</div>';
    lyricsDiv.html(html);
    lyricsDiv.slideDown();
